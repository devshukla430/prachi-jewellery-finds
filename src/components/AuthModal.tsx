...
export const AuthModal: React.FC = () => {
  ...
  const handleGoogleSignInClick = () => {
    ...
    if (!effectiveId) {
      setError('Google Sign-In is not configured yet. Please add NEXT_PUBLIC_GOOGLE_CLIENT_ID in configuration.');
      return;
    }

    // 1. Attempt Google Identity Services (GIS) Token Client popup first (no full page redirect)
    if (typeof window !== 'undefined' && (window as any).google?.accounts?.oauth2) {
      try {
          ...
          callback: async (tokenResponse: any) => {
            ...
            if (tokenResponse?.error) {
              if (tokenResponse.error === 'access_denied') {
                ...
                setError(`Google Sign-In: ${tokenResponse.error_description || tokenResponse.error}. If using a live domain, add https://prachi-jewellery-finds.vercel.app to Authorized JavaScript origins in Google Cloud Console.`);
              }
              return;
            }
            if (tokenResponse?.access_token) {
              try {
                ...
                if (userRes.ok) {
                  ...
                  if (profile.email) {
                    googleSignIn(profile.email, profile.name || profile.email.split('@')[0], profile.picture);
                    setIsAuthModalOpen(false);
                    resetAllForms();
                    return;
                  }
                ...
                console.warn('Failed to fetch Google profile info:', fetchErr);
              }
            }
          },
          error_callback: (err: any) => {
            setIsGoogleLoading(false);
            console.warn('GIS error, falling back to standard redirect:', err);
            initiateGoogleOAuth();
          },
        });
        ...
        console.warn('GIS initTokenClient error:', gisErr);
        setIsGoogleLoading(false);
      }
    }

    // 2. Fallback: standard OAuth 2.0 redirect
        ...
        {(mode === 'signin' || mode === 'signup') && (
          <>
            ...
            <button
              ...
              type="button"
              className="w-full py-2.5 px-4 rounded-full border border-[#F4D3DA] bg-[#FFF9FA] hover:bg-white text-xs font-medium text-[#2D2427] flex items-center justify-center gap-2.5 shadow-xs transition-colors disabled:opacity-60"
            >
              {isGoogleLoading ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin text-[#BA4A6E]" />
